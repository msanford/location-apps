...
class LocationVisitCell: UITableViewCell {
    ...
    private static let maxVenueNameLength = 30
    private static let truncateSuffix = "..."
    ...
    func updateUI(locationItem: BaseLocationItem) {
        ...
        if let venueName = locationItem.venueName {
            ...
            let maxPretruncationLength = LocationVisitCell.maxVenueNameLength - LocationVisitCell.truncateSuffix.characters.count
            // Truncate venue name to 30 chars to prevent wrapping
            if venueName.characters.count > maxPretruncationLength {
                let range = venueName.startIndex..<venueName.startIndex.advancedBy(maxPretruncationLength)
                truncatedVenueName = venueName.substringWithRange(range) + LocationVisitCell.truncateSuffix
            } else {
