...
class LocationMapController: UIViewController, MKMapViewDelegate, MFMailComposeViewControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        startPollingForLocationIfNeeded()
    }
    ...
    private func startPollingForLocationIfNeeded() {
        if (appDelegate.allLocationItems.count == 0) && (firstLocationTimer == nil) {
            // poll to get a location
            firstLocationTimer = NSTimer.scheduledTimerWithTimeInterval(3.0, target: self, selector: #selector(LocationMapController.onTimer(_:)), userInfo: nil, repeats: true)
        }
    }
    
    @objc private func onTimer(timer: NSTimer) {
        if case .AuthorizedAlways = CLLocationManager.authorizationStatus() {
            appDelegate.addCurrentPlace { [weak self] (coordindate, error) in
                if let coordindate = coordindate, strongSelf = self {
                    strongSelf.showMap(coordindate, animated: true)
                    timer.invalidate()
                    strongSelf.firstLocationTimer = nil
                }
            }
        }
    }
